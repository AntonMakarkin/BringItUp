      ...
      mailInputs.forEach(function (input) {
        input.addEventListener('keypress', function (e) {
          if (e.key.match(/[^a-z 0-9 @ \.]/ig)) {
            //russian letters are prohibited, only english
            e.preventDefault();
          }
        });
      });
...
function () {
  function VideoPlayer(triggers, overlay) {
    ...
    this.onPlayerStateChange = this.onPlayerStateChange.bind(this); //because every time we create new instance of the player, we losing the context
  }
  ...
  _createClass(VideoPlayer, [{
    ...
    value: function bindCloseBtn() {
      ...
      this.close.addEventListener('click', function () {
        ...
        //setTimeout(() => this.overlay.display = 'none', 400);
    ...
    value: function onPlayerStateChange(state) {
      try {
        var blockedElem = this.activeBtn.closest('.module__video-item').nextElementSibling; //the parent and sibling

        var playBtnSvg = this.activeBtn.querySelector('svg').cloneNode(true); //deep cloning of icon

        if (state.data === 0) {
          if (blockedElem.querySelector('.play__circle').classList.contains('closed')) {
            blockedElem.querySelector('.play__circle').classList.remove('closed');
            blockedElem.querySelector('svg').remove();
            blockedElem.querySelector('.play__circle').appendChild(playBtnSvg);
  ...
  _createClass(MainSlider, [{
    ...
    value: function showSlides(n) {
      ...
      this.slides.forEach(function (slide) {
        slide.style.display = 'none'; //slide.classList.add('animated', 'fadeIn');
      });
    ...
    value: function render() {
      if (this.container) {
        //if container with that selector exists
        try {
          this.hanson = document.querySelector('.hanson'); //block with your teacher on the third slide
        } catch (e) {}
